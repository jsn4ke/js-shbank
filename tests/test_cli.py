...
def test_fetch_command_exists():
    ...
    from js_shbank.cli import fetch_command
...
def test_web_command_exists():
    """测试 web_command 函数存在"""
    from js_shbank.cli import web_command

    assert callable(web_command)


@patch("js_shbank.cli.ApiClient")
@patch("js_shbank.cli.Parser")
@patch("js_shbank.cli.RepositoryFactory")
def test_fetch_command_flow(
    ...
    # 模拟命令行参数
    with patch("sys.argv", ["shbank-fetch", "--output", "test.csv"]):
        try:
            from js_shbank.cli import fetch_command
            sys.argv = ["shbank-fetch", "--output", "test.csv"]
        except SystemExit:
            pass


@patch("js_shbank.cli.subprocess")
